...
if (document.getElementById("simple-cal-export")) {
	...
	document
		.getElementById("simple-cal-export")
		.setAttribute("style", "height:50px; margin-left: 78.5px");
}
...
function buildGroupArray(els) {
	...
	for (let i = 0; i < els.length; i++) {
		let tempArr = [];
		try {
			href = els[i].children[1].children[0].children[1].href.match(
				/(?<=\meetup\.com\/)(.*?)(?=\s*\/)/
			)[0];
		} catch (err) {
			href = "";
...
function buttonInjection() {
	...
	let columnContainerCollection = document.getElementsByClassName(
	...
	function onClick() {
		...
		chrome.storage.local.set({
			dateRangeStart: startTime - 14400000,
			dateRangeEnd: startTime - 14100000,
		...
		columnContainerCollectionArray.map(x => {
			if (x.children[2].childElementCount === 1) {
				...
				buttonContainer.setAttribute(
					"style",
					'background-color: #0f1721;color: white;font-family: "Roboto", "Helvetica", "Arial", sans-serif;font-weight: 400; font-size: 14px;'
				);
...
chrome.extension.onMessage.addListener(function(request, sender, sendResponse) {
	if (request.type === "onUpdateFrmEvent") {
		sendResponse(
			"the the onupdate listener is working correctly and we recieved the message to fire our messages"
		);
	...
	if (document.getElementsByClassName("rock-salt")[0]) {
		...
		document
			...
			.setAttribute(
				"style",
				'margin: 0px 0px 0px 0px; padding-bottom: 15px; line-height: 26pt; font-family: "Graphik Meetup",helvetica,arial,sans-serif ; font-size: 28px; color: #00455d'
			);
		document
		...
		document
			...
			.setAttribute(
				"style",
				"position: -webkit-sticky; position: sticky; top: 135px; padding-bottom: 0px#b5d2c8; padding-bottom: 5px; box-shadow: 0px 0px 20px 3px rgba(0,0,0,0.64)"
			);
		document
			.getElementsByClassName("App")[0]
			.setAttribute(
				"style",
				"padding-top: 10px;padding-bottom: 0px;#e4e6e6; padding-bottom: 5px; box-shadow: 0px 0px 17px -7px rgba(0,0,0,0.23);"
			);
		document
			.getElementById("simple-event-filter-column")
			.setAttribute("style", `height: ${docHeight}px`);
